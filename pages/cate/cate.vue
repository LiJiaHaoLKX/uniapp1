		...
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="right-scroll-view">
			<view class="right-scroll-view-box" v-for="(item,index) in cateLevel2" :key="item.cat_id">
				...
				<view class="right-scroll-view-cateLevel3-box">
					<view class="right-scroll-view-cateLevel3" v-for="(item2,index) in item.children" @click="goToGoodsList(item2)">
					...
					item.children.forEach((item2)=>{
						if(item2.children){
							item2.children.forEach((item3)=>{
								item3.cat_icon='https://api-ugo-web.itheima.net/'+item3.cat_icon.split('/')[3]+'/'+item3.cat_icon.split('/')[4]
							})
						}
					})
				})
